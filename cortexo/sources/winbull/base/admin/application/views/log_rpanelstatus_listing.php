			...
			<div class="row">
				<div class="col-lg-12 grid-margin stretch-card">
					<div class="card admin_log">
						<div class="card-body">
							<h4 class="card-title"><i class="glyphicon glyphicon-th"></i> RPanel Status List</h4>
...
try {
		...
		table +='<thead><tr><th style="text-align:center">ID</th><th>Date</th><th>Rate Display</th><th>Market Status</th><th>Message</th></tr></thead><tbody>';
		$.ajax({						
			type: "POST",
			dataType: "json",					   
			url:"<?php echo $this->config->item('base_url')."index.php/C_admin_log/rpanelstatus_dataload/".$model_name; ?>/"+ document.getElementById('from_date').value+"/"+ document.getElementById('to_date').value,
			success: function(data)
			{
				var table_val='';
				$.each (data, function (i) {
				table_val += '<tr><td>'+data[i]['tracking_id']+'</td><td>'+data[i]['changetime']+'</td><td>'+data[i]['rate_display']+'</td><td>'+data[i]['market_status']+'</td><td>'+data[i]['market_message']+'</td></tr>';
				});
				$('#grid-data').remove();
