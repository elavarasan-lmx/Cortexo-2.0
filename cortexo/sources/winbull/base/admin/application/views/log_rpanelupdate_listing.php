			...
			<div class="row">
				<div class="col-lg-12 grid-margin stretch-card">
					<div class="card admin_log">
						<div class="card-body">
							<h4 class="card-title"><i class="glyphicon glyphicon-th"></i> Rpanel update List</h4>
							<p class="card-description"> </p>
							<div class="row form-sample1" style="margin-top:10px;margin-bottom:10px">
...
try {
		...
		table +='<thead><tr><th style="text-align:center">ID</th><th>Name</th><th>Date</th><th>Trade Type</th><th>Sell Diff</th><th>Buy Diff</th><th>Sell Rate</th></tr></thead><tbody>';
		$.ajax({						
			type: "POST",
			dataType: "json",					   
			url:"<?php echo $this->config->item('base_url')."index.php/C_admin_log/rpanelrate_dataload/".$model_name; ?>/"+ document.getElementById('from_date').value+"/"+ document.getElementById('to_date').value,
			success: function(data)
			{
				var table_val='';
				$.each (data, function (i) {
				table_val += '<tr><td>'+data[i]['tracking_id']+'</td><td>'+data[i]['rcom_disname']+'</td><td>'+data[i]['changetime']+'</td><td>'+data[i]['trade_type']+'</td><td>'+data[i]['sell_diff']+'</td><td>'+data[i]['buy_diff']+'</td><td>'+data[i]['sell_rate']+'</td></tr>';
				});
				$('#grid-data').remove();
