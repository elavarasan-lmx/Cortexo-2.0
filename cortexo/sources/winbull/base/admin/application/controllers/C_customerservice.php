<?php
class C_customerservice extends My_Controller {
	var $menu_code	= 27;
	var $form_entry = "customerservice_entry";	
	...
	function open_listingform($db_error_msg="") {		
		...
		{
			$this->load->view('customerservice_listing', $data);
		}
		else
		{
	...
	function open_entryform($model_name="",$type="",$id="") {			
		$model_name = 'customerservice_model'; // P-PERM fix: hardcode model name
	...
	{		
		...
		{									 
			...
			$data['error']="success";
			redirect("/C_customerservice/open_listingform/");																										   																						//Sending status to view as success.
		}
		else
		{		
