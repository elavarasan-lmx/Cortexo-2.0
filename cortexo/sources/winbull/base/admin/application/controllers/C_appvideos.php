<?php
class C_appvideos extends My_Controller {
	var $form_entry = "appvideo_entry";	
	public function __construct()
	{
		parent::__construct();
		$this->load->model("appvideo_model");
	}	
	...
	function open_listingform($db_error_msg="") {
		...
		$this->load->view('appvideo_listing',$data);
	}
	// Entry Form
	function open_entryform($model_name="",$type="",$id="") {			
		$model_name = 'appvideo_model'; // P-PERM fix: hardcode model name				
		if ($type=='add_new')	
		{							
	...
	{		
		...
		{									 
			...
			if($status=='add_new'){
				redirect("/C_appvideos/open_listingform");
			}
			else{
				redirect("/C_appvideos/open_listingform/");	
			}
