  ...
  app.get('/cron-jobs', async (request, reply) => {
    ...
    try {
      ...
      const mockJobs = [
        ...
        { id: 2, name: 'DB Backup', schedule: '0 0 * * *', command: 'mysqldump --all-databases | gzip > /backups/db-$(date +%Y%m%d).sql.gz', serverIp: '10.0.1.20', description: 'Nightly full database backup', isActive: true, timezone: 'Asia/Kolkata', lastRunAt: '2026-05-03T00:00:00Z', lastStatus: 'success', nextRunAt: '2026-05-04T00:00:00Z', totalRuns: 365, failedRuns: 0 },
