...
import { X, Users, Save, Loader2, Globe, Mail, Building } from 'lucide-react';

interface EditClientFormProps {
  client: { id: number; name: string; email: string; environment: string; domain: string; status?: string };
...
export default function EditClientForm({ client, onSave, onClose }: EditClientFormProps) {
  const [name, setName] = useState(client.name);
  const [email, setEmail] = useState(client.email);
  const [environment, setEnvironment] = useState(client.environment);
  ...
  const handleSubmit = async () => {
    setLoading(true);
    await onSave({ id: client.id, name, email, environment, domain, status });
    setLoading(false);
  };
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'rgb(var(--text-primary))', margin: 0 }}>Edit Client</h3>
              <p style={{ fontSize: '11px', color: 'rgb(var(--text-muted))', margin: 0 }}>{client.name}</p>
            </div>
          </div>
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            <label style={lbl}><Building style={{ width: '10px', height: '10px', display: 'inline', marginRight: '4px' }} />Client Name</label>
            <input value={name} onChange={e => setName(e.target.value)} style={inp} />
          </div>
          <div>
            <label style={lbl}><Mail style={{ width: '10px', height: '10px', display: 'inline', marginRight: '4px' }} />Contact Email</label>
          ...
          <div>
            <label style={lbl}><Globe style={{ width: '10px', height: '10px', display: 'inline', marginRight: '4px' }} />Domain</label>
