// Multi-format output formatter — table (TTY) / json (piping) / yaml (config files)
// Ported from AgentBrain's output-formatter.ts
...
export interface TableColumnDef {
  ...
  header: string;
  width?: number;
  /** Transform value before display (e.g. boolean → "yes"/"no") */
 ...
 * This enables: `cortexo server list | jq '.[] | .name'`
 */
...
/** Infer columns from first item's keys (auto-discovery, max 6 cols) */
function inferColumns(item: Record<string, unknown>): TableColumnDef[] {
  return Object.keys(item)
    .filter((k) => !['createdAt', 'updatedAt', 'deletedAt', 'orgId', 'created_at', 'updated_at', 'deleted_at', 'org_id'].includes(k))
    .slice(0, 6)
...
function formatValue(val: unknown): string {
  ...
  if (typeof val === 'boolean') return val ? chalk.green('yes') : chalk.red('no');
