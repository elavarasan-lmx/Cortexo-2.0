<?php
class C_rate_history extends My_Controller {
	var $form_entry = "ratehistory_entry";	
	public function __construct()
	{
		parent::__construct();	
		$this->load->model("ratehistory_model");		
	}	
	function index()
	{
	...
	function open_listingform($db_error_msg="") {		
		...
		$this->load->view('ratehistory_list', $data);
	}
	// Entry Form
	function open_entryform($model_name="",$type="",$id="") {			
		$model_name = 'ratehistory_model'; // P-PERM fix: hardcode model name				
		if ($type=='add_new')	
		{							
				$record					=	$this->$model_name->empty_record();
				$_POST['fv']['type']	=	$type;
				$_POST['fv']['rate_date']	=	date('d-m-Y');
				$_POST['fv']['rate_time']	=	date('h:i:s');
	...
	{		
		...
		{									 
			...
			$data['error']="success";
			redirect("/C_rate_history/open_listingform/");																										   																						//Sending status to view as success.
		}
		else
		{		
