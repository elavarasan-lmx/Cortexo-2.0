<?php
class C_whatsappmeta_settings extends My_Controller {
	var $entry_form = "whatsappmeta_settings_entry";	
	var $menu_code	= 81; // Keep same menu code for rights or update if different
	public function __construct()
	{
		parent::__construct();
		$this->load->helper("common");
		$this->load->model("Whatsappmeta_settings_model");
	}	
	function index()
	{
	...
	function open_listingform() {
		...
		{
			$this->load->view('whatsappmeta_listing',$data);
		}
		else
		{
	...
	function DB_Controller($model_name="",$status="",$id="")
	{
		...
		if($this->db->trans_status()===TRUE)
		{
			...
			if($status=='edit') {
				$this->session->set_flashdata('success', 'Meta Whatsapp settings updated successfully.');
			} else if($status=='delete') {
				$this->session->set_flashdata('success', 'Record deleted successfully.');
			}
			
			if ($is_ajax) {
				echo json_encode(["status" => "success", "message" => $this->session->flashdata('success'), "redirect" => site_url('C_whatsappmeta_settings/open_listingform')]);
				exit;
			}
			redirect("/C_whatsappmeta_settings/open_listingform/");
		}
		else
		{
			$this->db->trans_rollback();
			$db_error_msg = $this->general_model->get_errormessage($this->db->error()['code']);
			if($db_error_msg == "0") {
