<?php
class C_customergroup extends My_Controller
{
	var $form_entry = "customergroup_entry";
	var $menu_code	= 4;
	...
	function open_listingform($db_error_msg = "")
	{
		...
		$this->load->view('customergroup_listing', $data);
	}
	// Entry Form
	...
	function DB_Controller($model_name = "", $status = "", $id = "")	//Control DB Process and Validation Process.
	{
		...
		if ($this->db->trans_status() === TRUE) {
			...
			$data['error'] = "success";
			redirect("/C_customergroup/open_listingform/");																										   																						//Sending status to view as success.
		} else {
	...
	function customer_group($id, $type, $timestamp = '')
	{
		$result = $this->customergroup_model->update_record($id, $type);
		$this->session->set_flashdata('success', 'Record updated successfully.');
		echo $result;
	}
}
